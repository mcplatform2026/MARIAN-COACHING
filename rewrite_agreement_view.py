...
export function AgreementView() {
  ...
  const handleAgree = async () => {
    ...
    try {
      ...
      setAgreement((prev: any) => ({ ...prev, clientSignature: clientSig || agreement.clientSignature, status: 'accepted', acceptedAt: new Date().toISOString() }));
      setAccepted(true);
  ...
  if (loading && !agreement) {
    return <div className="min-h-screen flex items-center justify-center font-headline font-bold uppercase text-neutral-500 tracking-wider">Loading Agreement...</div>;
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-neutral-100">
        <div className="bg-white border-2 border-red-500 p-8 max-w-md w-full shadow-[8px_8px_0px_0px_rgba(239,68,68,1)]">
          ...
          <h2 className="text-red-700 font-headline font-black uppercase text-xl mb-2">Error</h2>
          ...
          {accepted && (
            <div className="mt-12 p-4 border-2 border-dashed" style={{ borderColor: currentTheme.border, backgroundColor: 'rgba(0,0,0,0.02)' }}>
               ...
               <p className="font-body text-[10px] opacity-70 leading-relaxed">
                 ...
                 Client signature securely captured and embedded directly into the document structure via secure database execution.
                 <br/><br/>
                 <strong>Created:</strong> {new Date(agreement.createdAt || Date.now()).toLocaleString()} <br/>
