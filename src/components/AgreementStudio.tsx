...
import { formatDateToMMDDYYYY, getCurrentDateMMDDYYYY } from '../utils/dateFormat';
import { AmericanDateInput } from './AmericanDateInput';
import { 
...
export function AgreementStudio({ onCancel, onCreate, brandColor, templates, initialData }: any) {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    const baseData = {
      ...initialData,
      agreementDate: initialData.agreementDate || formatDateToMMDDYYYY(initialData.createdAt || Date.now())
    };
    if (isNew && localStorage.getItem('agreementContentLocked') === 'true') {
      const savedContent = localStorage.getItem('defaultAgreementContent');
      if (savedContent) {
        return { ...baseData, projectDetails: savedContent };
      }
    }
    return baseData;
  });
  ...
  const handleSubmit = (e: React.FormEvent, isDraft: boolean) => {
    ...
    const finalData = {
      ...formData,
      agreementDate: formData.agreementDate || formatDateToMMDDYYYY(Date.now()),
      logoImage,
      invoiceTheme,
      invoiceTypography,
      ...
      <div className="w-full lg:w-[45%] flex flex-col gap-6 overflow-y-auto pr-2 pb-20">
        ...
        <div className="bg-white border-2 border-black p-4 neu-shadow-sm rounded-none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] font-headline font-bold uppercase tracking-wide text-neutral-600 mb-1">Agreement Date (MM/DD/YYYY)</label>
              <AmericanDateInput
                value={formData.agreementDate || ''}
                onChange={(val) => setFormData({ ...formData, agreementDate: val })}
                placeholder="MM/DD/YYYY"
                className="w-full text-sm p-2 border-2 border-black focus:outline-none bg-white font-body uppercase font-bold"
              />
            </div>
          </div>
        </div>
      ...
      <div className="w-full lg:w-[55%] h-full flex flex-col bg-surface-container-low border-2 border-black neu-shadow-sm overflow-hidden rounded-none relative">
        ...
        <div ref={previewContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden p-4 flex justify-center relative" style={{ backgroundColor: "#F0F3F6" }}>
          <div style={{ transform: `scale(${previewScale})`, transformOrigin: "top center", width: "794px", display: "flex", flexDirection: "column", marginBottom: `-${(1 - previewScale) * (pdfRef.current?.offsetHeight || 1123)}px` }}>
            <div ref={pdfRef} className="bg-white shadow-xl shrink-0 p-8 sm:p-12 md:p-14" style={{ width: '794px', minHeight: '1123px', position: 'relative' }}>
              {/* Header Block */}
              <div className="flex flex-row justify-between items-start pb-6 border-b-2 border-neutral-900 mb-6 gap-6 header-block">
                <div>
                  {logoImage ? (
                    ...
                    <div>
                      <div className="font-headline font-black text-2xl uppercase tracking-tight text-neutral-950" style={{ fontFamily: 'var(--font-headline-family)' }}>
                        MARIAN COACHING
                      </div>
              ...
              {/* Title */}
              <div className="mb-6">
                <h1 className="text-2xl sm:text-3xl font-headline font-black uppercase tracking-tight text-neutral-950 mb-1" style={{ fontFamily: 'var(--font-headline-family)' }}>
                  {formData.title || 'CLIENT SERVICES AGREEMENT'}
                </h1>
              ...
              <div className="border border-neutral-300 bg-neutral-50/70 p-5 mb-8 metadata-card">
                <div className="grid grid-cols-2 gap-x-8 gap-y-3.5 font-body">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-bold text-neutral-950 block">
                      {formData.providerSignatureLabel || 'Marian Coaching, LLC'}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-headline font-black uppercase tracking-wider text-neutral-500 block mb-0.5">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-black text-emerald-800 block">
                      {formData.fee || 'As specified in scope'}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-headline font-black uppercase tracking-wider text-neutral-500 block mb-0.5">
                    ...
                    <span className="text-sm font-bold text-neutral-900 block">
                      {formatDateToMMDDYYYY(formData.agreementDate || formData.createdAt || Date.now())}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-headline font-black uppercase tracking-wider text-neutral-500 block mb-0.5">
              ...
              <div className="agreement-prose mb-12">
                <style>{`
                  ...
                  .studio-tiptap h1 { font-family: var(--font-headline-family); font-size: 1.45em; font-weight: 900; text-transform: uppercase; color: #09090b; border-bottom: 1.5px solid #e4e4e7; padding-bottom: 0.3rem; }
                  .studio-tiptap h2 { font-family: var(--font-headline-family); font-size: 1.25em; font-weight: 800; text-transform: uppercase; color: #09090b; }
                  ...
                  .studio-tiptap ol { list-style-type: decimal; padding-left: 1.5em; margin: 0 !important; padding-top: 0.25em !important; padding-bottom: 0.25em !important; }
                `}</style>
              ...
              <div className="mt-14 pt-8 border-t-2 border-neutral-900 signatures-block">
                <h3 className="font-headline font-black uppercase text-base tracking-wider mb-1" style={{ fontFamily: 'var(--font-headline-family)' }}>
                ...
                <div className="grid grid-cols-2 gap-6 w-full">
                  <div className="flex flex-col border border-neutral-300 bg-neutral-50/40 p-4">
                    ...
                    <div className="w-full h-28 border-b-2 border-neutral-900 flex items-center justify-center bg-white px-2 py-1 mb-3">
                      {providerSig ? (
                        <img src={providerSig} alt="Provider Signature" className="max-h-20 max-w-full object-contain" />
                      ) : (
                        <div className="h-16 flex items-center justify-center text-xs opacity-40 uppercase italic font-headline">Not signed yet</div>
                      )}
                    </div>
                    <span className="font-headline font-bold text-xs uppercase text-neutral-950 truncate">
                      {formData.providerSignatureLabel || 'Service Provider'}
                    </span>
                    <span className="text-[11px] font-body text-neutral-500 mt-0.5">
                      Date: {formatDateToMMDDYYYY(formData.agreementDate || formData.createdAt || Date.now())}
                    </span>
                  </div>
                  <div className="flex flex-col border border-neutral-300 bg-neutral-50/40 p-4">
                    <span className="text-[10px] font-headline font-black uppercase tracking-wider text-neutral-500 mb-2">
                    ...
                    <div className="w-full h-28 border-b-2 border-neutral-900 flex items-center justify-center bg-white px-2 py-1 mb-3">
                      {clientSig ? (
                        <img src={clientSig} alt="Client Signature" className="max-h-20 max-w-full object-contain" />
                      ) : (
                        <div className="h-16 flex items-center justify-center text-xs opacity-40 uppercase italic font-headline">To be signed by client</div>
                      )}
                    </div>
                    <span className="font-headline font-bold text-xs uppercase text-neutral-950 truncate">
                      {formData.clientName || 'Client Name'}
                    ...
                    <span className="text-[11px] font-body text-neutral-500 mt-0.5">
                      Date: Pending Client Acceptance
                    </span>
                  </div>
                </div>
              </div>
