  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generateUniversalPDF({
        ...
        textColor: currentTheme?.text || '#000000',
        scale: 2,
        multiPage: true,
      });
    } catch (err) {
          ...
          {/* Header */}
              <div className="flex justify-between items-start mb-8 header-block">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs opacity-70 mt-1">{formatDateToMMDDYYYY(agreement.agreementDate || agreement.createdAt || Date.now())}</p>
          ...
          <div className="mt-16 pt-8 border-t-2" style={{ borderColor: currentTheme.border }}>
            <div className="flex flex-row gap-12 w-full">
              ...
              <div className="flex-1 w-1/2 flex flex-col items-center">
                ...
                {accepted && (
                  <p className="font-body text-xs mt-1 opacity-60">
                    {formatDateToMMDDYYYY(agreement.acceptedAt)}
                  </p>
                )}
              </div>
            </div>
